...
Register hooks on the core client for payment payload creation lifecycle events. Common use cases include enforcing spending limits, requiring approval for large payments or recording non-sensitive payment metadata.
...
<Warning>Never put complete payment payloads in logs or traces. Their signatures or authorizations may remain usable until settlement or expiry.</Warning>
